...
- **EMA safety guard:** The tuning logic ensures that `long_ema` never gets too close to or lower than `short_ema`, capping the minimum gap at 10 periods.
...
- BLEND logic now includes EMA knobs along with RSI, MACD, and cooldown.
- Logging improved during AutoTune to clarify blended values and clamped outcomes.
- Minor naming polish to reflect VODAN as the new executable name (`vodan` instead of `main.py`).
