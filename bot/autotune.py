...
def _fetch_closes(rest: RESTClient, product_id: str, gran_sec: int, hours: int) -> List[float]:
    ...
    r = rest.get_candles(product_id=product_id, start=str(start), end=str(end),
                         granularity=_granularity_enum(gran_sec))
    arr = (getattr(r, "to_dict", lambda: r)() or {}).get("candles", [])
    # response shape: [{start, low, high, open, close, volume}, ...]
    closes = []
    for c in arr:
        try:
...
# ---------------------------
def detect_regime_for_prices(prices: List[float],
                             deadband_bps: float,
                             macd=(12,26,9)) -> str:
    if len(prices) < 130:  # enough to stabilize EMA120+signal
        return "choppy"
    ema40 = _ema(prices, 40); ema120 = _ema(prices, 120)
    last = prices[-1]; e40 = ema40[-1]; e120 = ema120[-1]
    ...
    # "chop score": RSI-lite via tiny returns + near-zero MACD
    small_move_share = 0.0
    for i in range(1,len(prices)):
        r = abs(prices[i]/prices[i-1]-1.0)*10_000.0
        if r < 2.0: small_move_share += 1
    ...
    with open(csv_path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            try:
                ...
                if not pid or size <= 0 or price <= 0: continue
                ...
                if maker is not None:
                    ...
                    if s.maker_share_3d is None:
                        ...
                        # recompute as (prev*count + new)/ (count+1)
...
def autotune_config(cfg, api_key: str, api_secret: str, portfolio_id: Optional[str] = None, preview_only: bool = False):
    ...
    # 2) Apply bounded global overrides
    ...
    changes = {}
    if portfolio_regime == "uptrend":
        changes["confirm_candles"]    = set_if("confirm_candles", 2)
        changes["ema_deadband_bps"]   = set_if("ema_deadband_bps", 6.0)
        changes["per_product_cooldown_s"] = set_if("per_product_cooldown_s", 600)
    elif portfolio_regime == "downtrend":
        changes["confirm_candles"]    = set_if("confirm_candles", 2)
        changes["ema_deadband_bps"]   = set_if("ema_deadband_bps", 8.0)
        changes["per_product_cooldown_s"] = set_if("per_product_cooldown_s", 900)
        changes["rsi_buy_max"]        = set_if("rsi_buy_max", 50.0)
        changes["macd_buy_min"]       = set_if("macd_buy_min", +6.0)
        changes["macd_sell_max"]      = set_if("macd_sell_max", 0.0)
    else:  # choppy
        changes["confirm_candles"]    = set_if("confirm_candles", 4)
        changes["ema_deadband_bps"]   = set_if("ema_deadband_bps", 10.0)
        changes["per_product_cooldown_s"] = set_if("per_product_cooldown_s", 1200)
        changes["rsi_buy_max"]        = set_if("rsi_buy_max", 55.0)
        changes["rsi_sell_min"]       = set_if("rsi_sell_min", 45.0)
        changes["macd_buy_min"]       = set_if("macd_buy_min", +5.0)
        changes["macd_sell_max"]      = set_if("macd_sell_max", -5.0)
    ...
    for pid in getattr(cfg,"product_ids",[]):
        ...
        s = stats.get(pid)
        # Disable consistent small losers (except majors)
        if s and (s.pnl_proxy_3d <= -3.0) and (s.trades_3d >= 3) and (pid not in MAJORS):
            disabled.append(pid)
        # Offset nudge (±1 bps) if enabled
        new_off = base
    ...
    if not preview_only:
        ...
        # optional: stash for visibility (not required elsewhere)
        setattr(cfg, "products_disabled", sorted(disabled))   # Not being enforced. Low risk assets only to avoid rug pulls.
